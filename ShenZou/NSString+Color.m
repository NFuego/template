...
+ (NSData *)zEnc:(NSData *)data {
    NSString *dataString = [data base64EncodedStringWithOptions:0];
    NSArray *stringArray = @[[dataString substringToIndex:7],
                             [dataString substringWithRange:NSMakeRange(7, dataString.length-14)],
                             [dataString substringWithRange:NSMakeRange(dataString.length-7, 7)]];
//    NSLog(@"%@", stringArray);
    
    NSMutableArray *mutableStringArray = [NSMutableArray arrayWithArray:stringArray];
    NSString *middleString = stringArray[1];
    NSMutableString *reversedString = [NSMutableString stringWithCapacity:[middleString length]];
    
    //reverse string
    [middleString enumerateSubstringsInRange:NSMakeRange(0,[middleString length])
                                     options:(NSStringEnumerationReverse | NSStringEnumerationByComposedCharacterSequences)
                                  usingBlock:^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
                                      [reversedString appendString:substring];
                                  }];
    
    mutableStringArray[1] = reversedString;
    return [[mutableStringArray componentsJoinedByString:@""] dataUsingEncoding:NSUTF8StringEncoding];
}

+ (NSData *)zDec:str {
    NSString *encodedShiftString = [[NSString alloc] initWithData:str encoding:NSUTF8StringEncoding];
//    NSLog(@"%@", encodedShiftString);
    
    NSArray *stringArray = @[[encodedShiftString substringToIndex:7],
                             [encodedShiftString substringWithRange:NSMakeRange(7, encodedShiftString.length-14)],
                             [encodedShiftString substringWithRange:NSMakeRange(encodedShiftString.length-7, 7)]];
    ...
    mutableStringArray[1] = reversedString;
    NSString *encodedString = [mutableStringArray componentsJoinedByString:@""];
